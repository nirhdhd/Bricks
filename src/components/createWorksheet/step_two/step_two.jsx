  ...
  const fromInputToSpan = (inptVal) => {
    ...
    words.map(word => {
      ...
      word_span.addEventListener("click", function (e) {
        ...
        wordsStore = [...wordsStore, wordToInsert];
        wordsStoreToRender = wordsStore.map(x => <div id={x.key}> {x.value} </div>);
        // setWordsStoreToRender(wordsStore.map(x => <div id={x.key}> {x.value} </div>));
